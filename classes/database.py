# pylint:
# disable=import-error,lin:e-too-long,relative-import,logging-format-interpolation,broad-except
"""
    :contributors:
        - Daniel Quackenbush
"""
...
class Mongo():
    ...
    def check_local(self, host_dict):
        ...
        try:
            ...
            #logging.debug("No record in database. Ignoring")
            inventory = None
    ...
    def post(self, temp_dict, start):
        ...
        #logging.debug("X-RAY: Software upsert results {}".format(software_upsert))

        if host_dict['last_update_duration'] == -1:
            host_dict['last_update_duration'] = time.time() - start
        ...
        if host_remove.get("ok", 0) == 1:
            host_record = self.mongo.host_collection.insert(host_dict)
...
def compare_and_update_dicts(database, local):
    ...
    for software in local.get('software', {}):
        local_list += [(software['package_name'], version['name'],
                        version['install_date']) for version in software['versions']]
            ...
            if package['package_name'] == item[0]:
                ...
                if not version_exists:
                    ...
                    if package['package_name'].strip(
                        ...
                        logging.debug(
                            "X-RAY: New version detected for {}-{}".format(
                                package['package_name'], version.__dict__))
            else:
                software_counter += 1
